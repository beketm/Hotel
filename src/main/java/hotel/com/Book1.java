...
//	}
	...
	public List<List<String>> get_booking(String input) {
		   ...
		   String[] arr = Arrays.stream(input.split("&")).map(element -> element.split("=")[1].trim()).toArray(String[]::new);
		   ...
		   System.out.println(city_to_id.get(arr[0]));
		   List<List<String>> ans = dao.get_rooms_for_desk_clerk(city_to_id.get(arr[0]));
		   return ans;
	}
}
